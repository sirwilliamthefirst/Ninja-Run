...
def load_sprites(self):
        # Set run Sprites
        runSprites = sprite_loader.load_sprites(os.path.join(c.ASSETS_PATH, f"player_{self.player_num}"), "Run", RUN_SPRITE_FRAMES, c.SPRITE_WIDTH, c.SPRITE_HEIGHT)
        jumpSprites = sprite_loader.load_sprites(os.path.join(c.ASSETS_PATH, f"player_{self.player_num}"), "Jump", JUMP_SPRITE_FRAMES, c.SPRITE_WIDTH, c.SPRITE_HEIGHT)
...
class Player(pygame.sprite.Sprite): #maybe make an object class that player inherits that inherits sprites
    ...
    def load_sprites(self, color = "default"):
        ...
        if not self.sprites:
            print(f"Failed to load sprites for player {self.player_num} with color {self.color}")
            return False
        return True
...
class GlobalSpriteManager:
    ...
    def _scan_available_sprites(cls):
        ...
        common_colors = ["red", "blue", "green", "yellow", "purple", "orange", "pink", "cyan", "black", "white"]
        ...
        try:
            sprites = {
                'run': sprite_loader.load_sprites(
                    sprite_path, "Run", RUN_SPRITE_FRAMES, c.SPRITE_WIDTH, c.SPRITE_HEIGHT
                ),
                'jump': sprite_loader.load_sprites(
                    sprite_path, "Jump", JUMP_SPRITE_FRAMES, c.SPRITE_WIDTH, c.SPRITE_HEIGHT
                ),
                'attack': sprite_loader.load_sprites(
                    sprite_path, "Attack", ATTACK_SPRITE_FRAMES, c.SPRITE_WIDTH, c.SPRITE_HEIGHT
                )
            }
