...
class MapBuilder:
    def __init__(self, screen_width, screen_height, grid_x_units):
        ...
        self.tile_img = pygame.image.load(
            (os.path.join(c.ASSETS_PATH, "map/tile.png"))
        ).convert_alpha()
        self.background_img = pygame.image.load(
            (os.path.join(c.ASSETS_PATH, "map/background.png"))
        ).convert_alpha()
        self.background_img = pygame.transform.scale(
            self.background_img, c.BACKGROUND_IMAGE_DIMENSIONS
        )
        self.tree_img = pygame.image.load(
            (os.path.join(c.ASSETS_PATH, "map/tree.png"))
        ).convert_alpha()
        self.branch_img = pygame.image.load(
            (os.path.join(c.ASSETS_PATH, "map/branch.png"))
        ).convert_alpha()
        # Calculate number of background tiles needed
        self.num_background_tiles = (
            math.ceil(self.screen_width / c.BACKGROUND_IMAGE_DIMENSIONS[0]) + 1
        )
    ...
    def draw(self, screen):
        ...
        while i < self.num_background_tiles:
            screen.blit(
                self.background_img,
                [self.background_position + i * c.BACKGROUND_IMAGE_DIMENSIONS[0], 0],
            )
            i += 1
...
class Tree:
    ...
    def generate_branches(self, num_branches=None):
        ...
        mean = random.randint(
            c.BRANCH_HEIGHT_MEAN - c.BRANCH_HEIGHT_BOUND,
            c.BRANCH_HEIGHT_MEAN + c.BRANCH_HEIGHT_BOUND,
        )
        ...
        for i in range(num_branches):
            while True:
                ...
                if all(
                    abs(randomHeight - h) >= c.BRANCH_MIN_SPACING
        ...
        for randomHeight in branch_heights:
            ...
            branch = Branch(
                width, c.PLATFORM_HEIGHT, centerx, randomHeight, self.branch_img
            )
    ...
    def get_middle_branch(self):
        sorted_branches = sorted(
            self.branches, key=lambda branch: branch.get_top_center()[1]
        )
        if sorted_branches:
    ...
    def draw(self, screen):
        ...
        for branch in self.branches:
            branch.draw(screen)
            if c.DEBUG:
                # Draw the branch rectangle for debugging
                # pygame.draw.rect(screen, (0, 255, 0), branch.get_rect(), 2)
...
class Branch:
    def __init__(self, width, height, centerx, topy, branch_image=None):
        if branch_image is None:
            branch_image = pygame.image.load(
                (os.path.join(c.ASSETS_PATH, "map/branch.png"))
            ).convert_alpha()
        self.centerx = centerx
        self.img = pygame.transform.scale(
            branch_image, (width, height)
        )  # Flexible tile size
...
# List of Stages
GENERATION = {"forest": MapBuilder.generateForest}
