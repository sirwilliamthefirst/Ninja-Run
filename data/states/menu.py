...
class Menu(States):
    ...
    def startup(self):
        ...
        self.menu = pygame_menu.Menu(
            "Ninja Run", c.SCREEN_WIDTH, c.SCREEN_HEIGHT, theme=mytheme
        )
    ...
    def update(self, screen, dt):
        if self.visible_counter >= self.visible_switch:
            if self.player_enter_btn.get_title() == "":
                self.player_enter_btn.set_title(
                    "Press Enter/Start to join"
                )  # Show the label
            else:
    ...
    def add_player(self, joystick_id=None):
        if (
            len(States.players) == 0
        ):  # If this is the second player, get available colors
        ...
        decide_color = next(
            (
                ...
                if color not in self.in_use_colors
            ),
            None,
        )
        ...
        if joystick_id != None:
            ...
            joystick = next(
                stick for stick in States.joysticks if joystick_id == stick.id
            )
        else:
    ...
    def make_menu_buttons(self, menu):
        ...
        if len(States.players) == 1:
            ...
            menu.add.button(
                "Play (Co-op)", lambda: self.move_state(Game_States.GAME.value)
            )
...
class Leaderboard(States):
    ...
    def startup(self):
        ...
        self.menu = pygame_menu.Menu(
            "Leaderboard", c.SCREEN_WIDTH, c.SCREEN_HEIGHT, theme=mytheme
        )
